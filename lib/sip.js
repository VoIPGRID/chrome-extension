(function() {
    ...
    var code,
        ...
        reconnect,
        retry,
        retryTimeoutDefault = {interval: 2500, limit: 9000000},
        sipStack,
        states,
        status,
        subscriptions,
    ...
    function _getSIPmlVersion() {
        ...
        var pathPrefix = 'sipml5-',
            version;
        chrome.runtime.getManifest().background.scripts.forEach(function(file) {
    ...
    window.SIP = (function() {
            ...
            switch(code) {
                ...
                case tsip_event_code_e.STACK_FAILED_TO_START:
                    ...
                    if(reconnect) {
                        ...
                        retry = timer.increaseTimeout(retry);
                        console.info(`sip.increase_timeout - setting reconnect timeout to ${retry.timeout} ms`);
                        timer.setTimeout('contacts.reconnect', retry.timeout);
                ...
                case tsip_event_code_e.STACK_STARTED:
                    ...
                    timer.stopTimer('contacts.reconnect');
                    retry = timer.increaseTimeout(Object.assign({}, retryTimeoutDefault));
                    console.info(`sip.increase_timeout - resetted retry timer to ${retry.timeout} ms`);
                    timer.setTimeout('contacts.reconnect', retry.timeout);
                    break;
        ...
        var startStack = function() {
            var userAgent = 'Chrome extension v' + _getExtensionVersion() + ' w/ sipML5 v' + _getSIPmlVersion();
